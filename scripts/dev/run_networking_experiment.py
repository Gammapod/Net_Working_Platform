    ...
    *,
    ...
    for turn in range(1, turns + 1):
        ...
        if context["workflow_complete"]:
            transcript.append({"turn": turn, "actor_agent_id": actor_agent_id, "context": context, "raw_decision": None, "execution": None, "error": None, "skipped": True})
            continue
        raw = None
        execution = None
        error = None
        ...
        transcript.append({"turn": turn, "actor_agent_id": actor_agent_id, "context": context, "raw_decision": raw, "execution": execution, "error": error, "skipped": False})
...
def _prompt(context: dict[str, object]) -> str:
    return "\n".join([
        "You are in networking mode on Net Working Platform.",
        "Goal: choose contacts and open one relevant negotiation request. Do not conduct negotiation here.",
        "If you have a relevant contact and negotiation capacity, request_negotiation is the endpoint.",
        "If you do not have a contact, choose a useful same-field principal contact when available.",
